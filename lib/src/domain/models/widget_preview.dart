...
/// Which half of the widget gallery an entry belongs to. Drives the chip row at
/// the top of the index and how an entry opens: a component lists its cases
/// inline, a screen takes over the display.
enum WidgetSection {
...
/// One labelled rendering of a widget. The [label] is the specification, such
/// as `filled · large (52)`, and sits above what it describes. The [builder]
/// runs only once the case is on screen, so a stateful sample starts fresh
...
/// One entry in the widget gallery. Supplied by the host through
...
class WidgetPreview {
  ...
  /// Every labelled rendering, in the order the host declared them. A screen
  /// has exactly one, synthesised from its builder and named after the entry,
  /// so the screens that draw an entry need no second code path.
  ...
  /// Whether the entry can be listed at all. A malformed entry is dropped
  /// rather than thrown over, so one bad registration cannot stop the gallery
  /// from opening.
  bool get isUsable =>
